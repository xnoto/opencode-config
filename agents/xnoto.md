...
- `homebrew-opencode-agent-hub` and `opencode-agent-hub`: archived 2026-08-30 and no longer maintained; both repositories are read-only. The tap formula is deprecated (`:repo_archived`). Do not install, update, or extend them. Remove with `brew uninstall opencode-agent-hub && brew untap xnoto/opencode-agent-hub`; see the source repository README archive notice for APT/RPM/AUR and service removal.
