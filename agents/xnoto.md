...
- Add repo-only files to `.chezmoiexignore` under the always-ignore section unless they should be installed.
- Preserve platform behavior in `.chezmoiexignore` and templates: macOS uses `.zprofile`/AeroSpace and excludes Linux shell/i3/systemd paths; Linux uses `.bashrc.d`/i3 and excludes `.zprofile`/AeroSpace.
